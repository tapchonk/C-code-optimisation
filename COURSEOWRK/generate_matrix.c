...
{
  *A = (struct mesh *) _mm_malloc(sizeof(struct mesh),64); // Allocate matrix struct and fill it
  ...
  (*A)->ptr_to_vals_in_row = (float **) _mm_malloc(sizeof(float *) * local_nrow, 32);
  (*A)->ptr_to_inds_in_row = (int **) _mm_malloc(sizeof(int *) * local_nrow, 32);
  (*A)->ptr_to_diags = (float **) _mm_malloc(sizeof(float *) * local_nrow, 32);
  ...
  *xexact = (double *) _mm_malloc(sizeof(double) * local_nrow, 32);
  ...
  (*A)->list_of_vals = (float *) _mm_malloc(sizeof(float) * local_nnz, 32);
